...
from onvif_utils import get_rtsp_uri, get_device_information
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.table.setHorizontalHeaderLabels(
            ["IP", "Тип", "Производитель", "Имя", "Источник", "Статус", "Данные", "Действия"]
        )
    ...
    def open_camera(self, row: int):
        ...
        if camera.onvif_url:
            ...
            try:
                ...
                try:
                    info = get_device_information(camera.ip, port, username, password)
                    camera.manufacturer = camera.manufacturer or info.get("manufacturer")
                    camera.model = camera.model or info.get("model")
                    camera.firmware = camera.firmware or info.get("firmware")
                    camera.serial_number = camera.serial_number or info.get("serial_number")
                    camera.hardware_id = camera.hardware_id or info.get("hardware_id")
                except Exception:
                    pass
                self.render_cameras()
                self.open_video(camera.rtsp_url)
    ...
    def open_video(self, rtsp_url: str):
        ...
        window.destroyed.connect(lambda: self._cleanup_video_window(window))
        self.video_windows.append(window)
        window.show()

    def _cleanup_video_window(self, window: VideoWindow):
